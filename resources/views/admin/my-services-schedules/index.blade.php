                ...
                <tbody>
                    @forelse($appointments as $appointment)
                        @php
                            ...
                            $patientName = $composedName ?: ($patientProfile?->full_name ?? $appointment->patient->name ?? 'N/A');
                            ...
                            $dateInputValue = $appointment->scheduled_date
                                ? \Carbon\Carbon::parse($appointment->scheduled_date)->format('Y-m-d')
                                : optional($appointment->created_at)->format('Y-m-d');
                            $timeInputValue = $appointment->scheduled_time
                                ? (strlen($appointment->scheduled_time) >= 5 ? substr($appointment->scheduled_time, 0, 5) : $appointment->scheduled_time)
                                : '';
                        @endphp
                        <tr>
                            <td>
        ...
        <div class="patient-modal-body">
            ...
            <div class="patient-form-section">
                ...
                <div class="patient-form-section" style="margin-top: 1rem;">
                    ...
                    <div style="margin-bottom: 0.75rem;"><strong>Before:</strong> <span id="modal-progress-before-list" style="display: flex; flex-wrap: wrap; gap: 0.5rem;"></span></div>
                    <div style="margin-bottom: 0.75rem;"><strong>After:</strong> <span id="modal-progress-after-list" style="display: flex; flex-wrap: wrap; gap: 0.5rem;"></span></div>
                    <form id="progressPhotosForm" enctype="multipart/form-data" style="margin-top: 0.5rem;" data-progress-url="{{ url('/admin/my-services-schedules') }}">
                        <div class="patient-form-row">
                            <div class="patient-form-group">
                    ...
                    <p style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">These photos are for viewing on this appointment. Use &quot;Add Result&quot; when ready to save to patient history.</p>
                </div>
            </div>
...
<div id="doneConfirmModal" class="patient-modal">
    <div class="patient-modal-content" style="max-width: 440px;">
        <div class="patient-modal-header">
            ...
            <button class="patient-modal-close" type="button" onclick="closeDoneConfirmModal()">×</button>
        </div>
        <div class="patient-modal-body" style="padding: 1.5rem 2rem;">
...
function populatePatientModal(data) {
    ...
    document.getElementById('modal-consultation-referral-source').value = appointmentData.referral_source || '';
    
    const conditionPhotosDiv = document.getElementById('modal-condition-photos');
    if (conditionPhotosDiv) {
        conditionPhotosDiv.innerHTML = '';
        (appointmentData.condition_photos || []).forEach(function(path) {
        ...
        document.getElementById('modal-condition-photos-wrap').style.display = (appointmentData.condition_photos && appointmentData.condition_photos.length) ? 'block' : 'none';
    }
    const storageUrl = '{{ asset("storage") }}';
    const beforeList = document.getElementById('modal-progress-before-list');
    const afterList = document.getElementById('modal-progress-after-list');
    const progressPhotos = appointmentData.progress_photos || { before: [], after: [] };
    if (beforeList) {
        beforeList.innerHTML = '';
        (progressPhotos.before || []).forEach(function(path) {
            const img = document.createElement('img');
    ...
    if (afterList) {
        afterList.innerHTML = '';
        (progressPhotos.after || []).forEach(function(path) {
            const img = document.createElement('img');
            img.src = storageUrl + '/' + path;
            img.alt = 'After';
            img.style.cssText = 'max-width: 120px; max-height: 120px; object-fit: cover; border-radius: 4px;';
...
// Handle status/date/time updates for service schedules (admin)
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.admin-schedule-status-select').forEach(function(sel) {
        sel.addEventListener('change', function() {
            ...
            patchServiceSchedule(id, { status }).then(function(res) {
    ...
    document.getElementById('doneConfirmNoBtn').addEventListener('click', function() {
        ...
        patchServiceSchedule(appointmentId, { status: 'completed' }).then(function(res) {
    ...
    document.querySelectorAll('.admin-schedule-edit-date').forEach(function(inp) {
        inp.addEventListener('change', function() {
            patchServiceSchedule(this.getAttribute('data-appointment-id'), { scheduled_date: this.value }).then(function(res) {
                if (!res.success) alert(res.message || 'Failed to update date');
            }).catch(function() { alert('Failed to update date'); });
        });
    });
    document.querySelectorAll('.admin-schedule-edit-time').forEach(function(inp) {
        inp.addEventListener('change', function() {
            patchServiceSchedule(this.getAttribute('data-appointment-id'), { scheduled_time: this.value }).then(function(res) {
    ...
    if (progressPhotosForm) {
        progressPhotosForm.addEventListener('submit', function(e) {
            e.preventDefault();
            if (!currentModalAppointmentId) return;
            ...
            const baseUrl = this.getAttribute('data-progress-url');
            fetch(`${baseUrl}/${currentModalAppointmentId}/progress-photos`, {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                body: formData
            }).then(r => r.json()).then(function(res) {
                if (res.success) {
                    openPatientModal(currentModalAppointmentId);
