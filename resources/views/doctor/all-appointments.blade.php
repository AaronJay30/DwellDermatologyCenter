...
<style>
    ...
    .file-preview-item .file-caption {
        padding: 0.5rem;
        font-size: 0.75rem;
        color: #333;
    ...
    .file-preview-item .file-caption input {
        width: 100%;
        padding: 0.25rem;
...
@section('content')
@php
    $hasPastAppointments = $appointments->contains(function ($appointment) {
        $appointmentDate = $appointment->timeSlot->date
            ?? $appointment->doctorSlot->slot_date
            ?? ($appointment->scheduled_date ? \Carbon\Carbon::parse($appointment->scheduled_date) : null)
            ?? $appointment->created_at;

        return $appointmentDate
            ? \Carbon\Carbon::parse($appointmentDate)->lt(\Carbon\Carbon::today())
            : false;
    });
@endphp
<div class="container">
    <div class="slots-header">
        <div class="slots-header-left">
            <span class="management-label">ALL APPOINTMENTS</span>
        </div>
        <div class="slots-header-right">
            <form method="GET" action="{{ route('doctor.all-appointments') }}" class="search-wrapper" id="searchForm">
    ...
    {{-- Past appointments notification modal --}}
    <div id="pastNoticeModal" class="patient-modal" style="display: none;">
        <div class="patient-modal-content" style="max-width: 540px;">
            <div class="patient-modal-header">
                <h2 class="result-modal-title" style="margin: 0;">Heads up</h2>
                <button class="patient-modal-close" onclick="closePastNoticeModal()">&times;</button>
            </div>
            <div class="patient-modal-body" style="padding-top: 1rem;">
                <p style="margin: 0 0 1rem 0; color: #374151;">
                    Some bookings are already in the past and have no results yet.
                    You can add a result or use <strong>Cancel &amp; Delete</strong> to clear them.
                </p>
                <div style="display: flex; justify-content: flex-end; gap: 0.5rem;">
                    <button class="patient-modal-btn patient-modal-btn-cancel" onclick="closePastNoticeModal()">OK</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="deleteConfirmModal" class="patient-modal" style="display: none;">
        <div class="patient-modal-content" style="max-width: 520px;">
            <div class="patient-modal-header">
                <h2 class="result-modal-title" style="margin: 0;">Cancel & Delete</h2>
                <button class="patient-modal-close" onclick="closeDeleteModal()">&times;</button>
            </div>
            <div class="patient-modal-body" style="padding-top: 1rem;">
                <p style="margin: 0 0 1rem 0; color: #374151;">
                    This will cancel the booking and remove it from the patient’s records. The patient will be notified that they missed the appointment.
                </p>
                <div id="deleteStatusMessage" style="color: #b91c1c; font-size: 0.9rem; margin-bottom: 0.5rem; display: none;"></div>
                <div style="display: flex; justify-content: flex-end; gap: 0.5rem;">
                    <button class="patient-modal-btn patient-modal-btn-cancel" onclick="closeDeleteModal()">Keep Booking</button>
                    <button id="confirmDeleteBtn" class="patient-modal-btn patient-modal-btn-download" style="background-color: #dc3545;" onclick="confirmDelete()">Yes, Cancel & Delete</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    @forelse($appointments as $appointment)
                        @php
                            ...
                            $appointmentDate = $appointment->timeSlot->date
                                ...
                                ?? ($appointment->scheduled_date ? \Carbon\Carbon::parse($appointment->scheduled_date) : null)
                                ?? $appointment->created_at;
                            $isPast = $appointmentDate ? \Carbon\Carbon::parse($appointmentDate)->lt(\Carbon\Carbon::today()) : false;
                        @endphp
                        <tr>
                            <td>
        ...
        <form id="addResultForm" enctype="multipart/form-data">
            ...
            <div class="result-modal-body">
                ...
                <div class="result-form-section">
                    <div class="result-section-header">📸 BEFORE PHOTOS</div>
                    
                    <div class="result-form-group">
                        <label>Upload Before Photos (Multiple)</label>
                        <div class="file-upload-area" id="beforeFilesArea">
                            <div class="file-upload-buttons">
                                <input type="file" name="before_files[]" id="beforeFiles" multiple accept="image/*" style="display: none;">
                ...
                <div class="result-form-section">
                    <div class="result-section-header">🩺 BEFORE CONSULTATION FINDINGS</div>
                    
                    <div class="result-form-group">
                        <label>Findings (Multiple Bullet Points)</label>
                        <div id="beforeFindingsContainer">
                            <div class="bullet-note-item">
                                <span class="bullet-point">•</span>
                                <input type="text" name="before_findings[]" class="bullet-input" placeholder="Enter finding...">
                                <button type="button" class="btn-remove-bullet" onclick="removeBulletItem(this)" style="display: none;">×</button>
                            </div>
                        </div>
                        <button type="button" class="btn-add-bullet" onclick="addBulletItem('beforeFindingsContainer')" style="margin-top: 0.5rem; padding: 0.5rem 1rem; background-color: #008080; color: white; border: none; border-radius: 4px; cursor: pointer;">Add Another</button>
                    </div>
                </div>
                
                <!-- AFTER PHOTOS Section -->
                <div class="result-form-section">
                    <div class="result-section-header">📸 AFTER PHOTOS</div>
                    
                    <div class="result-form-group">
                        <label>Upload After Photos (Multiple)</label>
                ...
                <div class="result-form-section">
                    <div class="result-section-header">🧪 AFTER CONSULTATION RESULTS</div>
                    
                    <div class="result-form-group">
                        <label>Results (Multiple Bullet Points)</label>
                        <div id="afterResultsContainer">
                            <div class="bullet-note-item">
                                <span class="bullet-point">•</span>
                                <input type="text" name="after_results[]" class="bullet-input" placeholder="Enter result...">
                                <button type="button" class="btn-remove-bullet" onclick="removeBulletItem(this)" style="display: none;">×</button>
                            </div>
                        </div>
                        <button type="button" class="btn-add-bullet" onclick="addBulletItem('afterResultsContainer')" style="margin-top: 0.5rem; padding: 0.5rem 1rem; background-color: #008080; color: white; border: none; border-radius: 4px; cursor: pointer;">Add Another</button>
                    </div>
                </div>
                
                <!-- PRESCRIPTION Section -->
                <div class="result-form-section">
                    <div class="result-section-header">💊 PRESCRIPTION</div>
                    
                    <div class="result-form-group">
                        <label>Prescription Items (Multiple Bullet Points)</label>
                        <div id="prescriptionContainer">
                            <div class="bullet-note-item">
                                <span class="bullet-point">•</span>
                                <input type="text" name="prescription[]" class="bullet-input" placeholder="Enter prescription item...">
                                <button type="button" class="btn-remove-bullet" onclick="removeBulletItem(this)" style="display: none;">×</button>
                            </div>
                        </div>
                        <button type="button" class="btn-add-bullet" onclick="addBulletItem('prescriptionContainer')" style="margin-top: 0.5rem; padding: 0.5rem 1rem; background-color: #008080; color: white; border: none; border-radius: 4px; cursor: pointer;">Add Another</button>
                    </div>
                </div>
                
                <!-- MEDICATIONS TO TAKE Section -->
                <div class="result-form-section">
                    <div class="result-section-header">💧 MEDICATIONS TO TAKE</div>
                    
                    <div class="result-form-group">
                        <label>Oral Medications (Multiple Bullet Points)</label>
                        <div id="medicationsToTakeContainer">
                            <div class="bullet-note-item">
                                <span class="bullet-point">•</span>
                                <input type="text" name="medications_to_take[]" class="bullet-input" placeholder="Enter medication...">
                                <button type="button" class="btn-remove-bullet" onclick="removeBulletItem(this)" style="display: none;">×</button>
                            </div>
                        </div>
                        <button type="button" class="btn-add-bullet" onclick="addBulletItem('medicationsToTakeContainer')" style="margin-top: 0.5rem; padding: 0.5rem 1rem; background-color: #008080; color: white; border: none; border-radius: 4px; cursor: pointer;">Add Another</button>
                    </div>
                </div>
                
                <!-- NOTES Section -->
                <div class="result-form-section">
                    <div class="result-section-header">📝 NOTES</div>
                    
                    <div class="result-form-group">
                        <label for="notes">Notes (Optional)</label>
                        <textarea name="notes" id="notes" rows="6" placeholder="Enter any additional notes..."></textarea>
                    </div>
                </div>
                
                <!-- FOLLOW-UP DATE Section -->
                <div class="result-form-section">
                    <div class="result-section-header">📅 FOLLOW-UP DATE</div>
                    
                    <div class="result-form-group">
                        <label for="followUpDate">Follow-up Date (Optional)</label>
                        <input type="date" name="follow_up_date" id="followUpDate" class="result-form-input">
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const hasPastAppointments = @json($hasPastAppointments);
...
function openDeleteModal(appointmentId) {
    ...
    const status = document.getElementById('deleteStatusMessage');
    ...
    if (btn) {
        btn.disabled = false;
        btn.textContent = 'Yes, Cancel & Delete';
    }
...
function confirmDelete() {
    ...
    fetch(`{{ url('/doctor/all-appointments') }}/${appointmentToDelete}`, {
        method: 'DELETE',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (status) {
                status.textContent = data.message || 'Error deleting appointment.';
    ...
    .catch(() => {
        if (status) {
            status.textContent = 'An error occurred while deleting the appointment.';
            status.style.display = 'block';
        }
        if (btn) {
            btn.disabled = false;
...
function openPastNoticeModal() {
    const modal = document.getElementById('pastNoticeModal');
    if (modal) {
        modal.style.display = 'flex';
        modal.classList.add('active');
        document.body.style.overflow = 'hidden';
...
function handleFileSelect(event, previewId, inputId) {
    ...
    files.forEach((file, index) => {
        ...
        reader.onload = function(e) {
            ...
            previewItem.dataset.fileIndex = Date.now() + index; // Unique index for this file
            ...
            if (file.type.startsWith('image/')) {
                previewItem.innerHTML = `
                    ...
                    <div class="file-caption">
                        <input type="text" name="file_captions[${inputId}][${previewItem.dataset.fileIndex}]" placeholder="Optional caption..." class="caption-input">
                    </div>
...
function openCamera(inputId) {
    ...
    if (input) {
        ...
        // Keep accept attribute to allow images only
...
function getFieldName(containerId) {
    if (containerId === 'beforeFindingsContainer') return 'before_findings';
    if (containerId === 'afterResultsContainer') return 'after_results';
    if (containerId === 'prescriptionContainer') return 'prescription';
    if (containerId === 'medicationsToTakeContainer') return 'medications_to_take';
    return 'notes';
}

function resetBulletNotes() {
    // Reset all bullet note containers to have one empty item
    ['beforeFindingsContainer', 'afterResultsContainer', 'prescriptionContainer', 'medicationsToTakeContainer'].forEach(containerId => {
        const container = document.getElementById(containerId);
        if (container) {
            ...
            container.innerHTML = `
                <div class="bullet-note-item">
                    <span class="bullet-point">•</span>
                    <input type="text" name="${fieldName}[]" class="bullet-input" placeholder="Enter item...">
                    <button type="button" class="btn-remove-bullet" onclick="removeBulletItem(this)" style="display: none;">×</button>
                </div>
            `;
        }
...
function deleteAppointment(appointmentId) {
    ...
    fetch(`{{ url('/doctor/all-appointments') }}/${appointmentId}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': csrfToken,
            'Content-Type': 'application/json',
...
function handleResultSubmit(e) {
    ...
    formData.delete('before_findings[]');
    formData.delete('after_results[]');
    formData.delete('prescription[]');
    formData.delete('medications_to_take[]');
    
    // Combine bullet notes into single strings (only set if not empty)
    const beforeFindingsArray = Array.from(form.querySelectorAll('input[name="before_findings[]"]'))
        .map(input => input.value.trim())
        .filter(val => val !== '');
    if (beforeFindingsArray.length > 0) {
        formData.set('before_findings', '• ' + beforeFindingsArray.join('\n• '));
    }
    
    const afterResultsArray = Array.from(form.querySelectorAll('input[name="after_results[]"]'))
        .map(input => input.value.trim())
        .filter(val => val !== '');
    if (afterResultsArray.length > 0) {
        formData.set('after_results', '• ' + afterResultsArray.join('\n• '));
    }
    
    const prescriptionArray = Array.from(form.querySelectorAll('input[name="prescription[]"]'))
        .map(input => input.value.trim())
        .filter(val => val !== '');
    if (prescriptionArray.length > 0) {
        formData.set('prescription', '• ' + prescriptionArray.join('\n• '));
    }
    
    const medicationsToTakeArray = Array.from(form.querySelectorAll('input[name="medications_to_take[]"]'))
        .map(input => input.value.trim())
        .filter(val => val !== '');
    if (medicationsToTakeArray.length > 0) {
        formData.set('medications_to_take', '• ' + medicationsToTakeArray.join('\n• '));
    }
    ...
    fetch(`{{ url('/doctor/all-appointments') }}/${currentAppointmentId}/result`, {
        method: 'POST',
        headers: {
            'Accept': 'application/json'
        },
