    ...
    <div class="category-form-container compact-form">
        <div class="form-header">
            <div>
                ...
                <p style="margin: 0.35rem 0 0; color: #6c757d;">Update the category details and assign it to the right clinic.</p>
            </div>
            <a href="{{ route('doctor.categories') }}" class="btn btn-accent" style="padding: 0.6rem 1.5rem;">Back to Categories</a>
        </div>

        <form method="POST" action="{{ route('doctor.categories.update', $category) }}" enctype="multipart/form-data" id="doctorCategoryEditForm">
            @csrf
            @method('PUT')
            ...
            <div class="modern-input-wrapper">
                ...
                <div class="image-upload-area" id="doctorCategoryEditImageUploadArea">
                ...
                @error('image')
                    <span class="error-message">{{ $message }}</span>
                @enderror
                <div id="doctorCategoryEditImagePreview" class="category-image-preview-container">
                    @if($category->image_path)
                        <img id="doctorCategoryEditImagePreviewImg" src="{{ asset('storage/'.$category->image_path) }}" alt="Image preview">
                    @else
                        <img id="doctorCategoryEditImagePreviewImg" src="#" alt="Image preview">
                    @endif
                </div>
            </div>
            ...
            <div class="form-actions" style="margin-top: 1.5rem;">
                ...
                <a href="{{ route('doctor.categories') }}" class="btn btn-accent" style="padding: 0.6rem 1.5rem; font-size: 0.95rem;">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const imageUploadArea = document.getElementById('doctorCategoryEditImageUploadArea');
    const uploadButton = imageUploadArea ? imageUploadArea.querySelector('.upload-button') : null;
    const previewContainer = document.getElementById('doctorCategoryEditImagePreview');
    const previewImg = document.getElementById('doctorCategoryEditImagePreviewImg');
