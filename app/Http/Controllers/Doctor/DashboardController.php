...
class DashboardController extends Controller
{
    ...
    public function getAdminReports(Request $request, $adminId)
    {
        ...
        if ($request->filled('service_type')) {
            if ($request->service_type === 'consultation') {
                $query->whereNull('service_id')->whereNotNull('consultation_type');
            } else {
        ...
        if ($sortColumn === 'patient_name') {
            $query->leftJoin('users', 'appointments.patient_id', '=', 'users.id')
                  ->orderBy('users.name', $sortDirection)
                  ->select('appointments.*');
        } elseif ($sortColumn === 'date') {
            $query->leftJoin('time_slots', 'appointments.time_slot_id', '=', 'time_slots.id')
                  ->orderBy('time_slots.date', $sortDirection)
                  ->orderBy('time_slots.start_time', $sortDirection)
                  ->select('appointments.*');
        } else {
            $query->orderBy('appointments.' . $sortColumn, $sortDirection);
        }
