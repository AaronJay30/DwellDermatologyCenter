...
class PromoController extends Controller
{
    ...
    protected function ensureServiceBelongsToBranch(int $serviceId, int $branchId): void
    {
        $belongs = Service::where('id', $serviceId)
            ->whereHas('category', function ($query) use ($branchId) {
                $query->where('branch_id', $branchId);
            })
            ->exists();

        if (!$belongs) {
            abort(403, 'You are not allowed to use this service.');
        }
    }
    ...
    public function index()
    {
        ...
        // Auto-expire promotions for this branch
        Promotion::where('branch_id', $branch->id)
    ...
    public function store(Request $request)
    {
        ...
        foreach ($selectedServices as $index => $serviceData) {
            ...
            // Validate that each selected service belongs to the admin's branch
            $this->ensureServiceBelongsToBranch((int) $serviceData['service_id'], $branch->id);
        }
    ...
    public function edit(Promotion $promo)
    {
        ...
        // Check if promotion belongs to admin's branch
        if ((int) $promo->branch_id !== (int) $branch->id) {
            abort(403, 'You are not allowed to edit this promotion.');
        }

        $promo->load(['images', 'promoServices.service', 'branch']);
        $services = Service::where('is_active', true)
            ->whereHas('category', function ($query) use ($branch) {
                $query->where('branch_id', $branch->id);
            })
    ...
    public function update(Request $request, Promotion $promo)
    {
        ...
        // Validate that each service belongs to the admin's branch
        foreach ($validated['services'] as $serviceData) {
            $this->ensureServiceBelongsToBranch((int) $serviceData['service_id'], $branch->id);
        }
    ...
    public function destroy(Promotion $promo)
    {
        ...
        // Check if promotion belongs to admin's branch
        if ((int) $promo->branch_id !== (int) $branch->id) {
            abort(403, 'You are not allowed to delete this promotion.');
        }
